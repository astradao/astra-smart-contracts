...
interface IUniswapV2Pair {
    function MINIMUM_LIQUIDITY(address pair) external pure returns (uint256);
...
contract mockSwap is Ownable {
    ...
    uint256 public swapResultIndex;

    function initialize(address _owner, address _sushiAddress, address _univ2Address, address _univ3Address, address _quoterAddress) public initializer {
        Ownable.init(_owner);
        ...
        quoterV3 = IQuoter(_quoterAddress);
        poolFee = 3000;
    }
    ...
    function getPairRateUniV3(
        ...
        if (UniversalERC20.isETH(_tokenIn)) {
            path[0] = uniswapRouterV3.WETH9();
            path[1] = address(_tokenOut);
        } else if (UniversalERC20.isETH(_tokenOut)) {
            path[0] = address(_tokenIn);
            path[1] = uniswapRouterV3.WETH9();
        } else {
            path[0] = address(_tokenIn);
            path[1] = address(_tokenOut);
        }
        ...
        uint256 _tokenInSwapAmount = getReserveV3(pairAddress, IERC20(path[0]));

        if(_amountIn > _tokenInSwapAmount) {
            return 0;
        }
    ...
    function getBestExchangeRate(
        ...
        IERC20 _tokenOut,
        uint256 _amountIn
    ) public returns (uint256 amountOut, uint8 platform) {
    ...
    {
        IUniswapV2Pair pair = IUniswapV2Pair(address(uniswapRouterV2));
        (uint112 liq1, uint112 liq2, ) = pair.getReserves(pairAddress);
        _tokenIn = pair.token0();
        _tokenInBal = uint256(liq1).sub(pair.MINIMUM_LIQUIDITY(pairAddress));
        _tokenoutBal = uint256(liq2).sub(pair.MINIMUM_LIQUIDITY(pairAddress));
    }
    ...
    function getReserveV3(address pair, IERC20 token0)
        ...
        returns (uint256 token0Bal)
    {
        // token0Bal = uniswapRouterV3.getReserveV3(pair, token0);
        if(pair == msg.sender){
           token0Bal = 0;
        }else{
    ...
    function convertExactEthToTokensV3(
        IERC20 _tokenOut,
        uint256 _amountOutMinimum
    ) internal {
        ...
        uniswapRouterV3.exactInputSingle{value: msg.value}(params);
    ...
    function convertExactTokensToEthV3(
        IERC20 _tokenIn,
        IERC20 _tokenOut,
        uint256 _amountIn,
        uint256 _amountOutMinimum
    ...
    function uniSwapV3(
        ...
        if (UniversalERC20.isETH(_tokenIn)) {
            convertExactEthToTokensV3(_tokenOut, _amountOutMinimum);
        } else {
            convertExactTokensToEthV3(
                _tokenIn,
                _tokenOut,
                _amountIn,
                _amountOutMinimum
    ...
    function swapFromBestExchange(
        ...
        if (_exchange == 0) {
            revert("Not enough liquidity in available swaps");
        }else if (_exchange == 1) {

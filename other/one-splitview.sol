...
contract OneSplitRoot is IOneSplitView {
    ...
    IBancorEtherToken constant internal bancorEtherToken = IBancorEtherToken(0x7816fBBEd2C321c24bdB2e2477AF965Efafb7aC0);
    IWETH constant internal weth = IWETH(0x7816fBBEd2C321c24bdB2e2477AF965Efafb7aC0);
    ...
    IMooniswapRegistry constant internal mooniswapRegistry = IMooniswapRegistry(0x642f6B988BbDB5AA778c5120bEc90BFb1aAaaa64);
...
contract OneSplitDMMBase {
    IDMMController internal constant _dmmController = IDMMController(0xBb3B71e833792A444e728c42B23EaeA242c4702d);
    ...
    OneSplitMooniswapToken
{
    ...
    function swapMulti(
        ...
        // Transfer From
        tokens[0].universalTransferFromSenderToThis(amount);
        uint256 confirmed = tokens[0].universalBalanceOf(address(this)).sub(beforeBalances.ofFromToken);
        tokens[0].universalApprove(address(oneSplit), confirmed);
        
        oneSplit.swap.value(tokens[0].isETH() ? amount : 0)(
            tokens[0],
            tokens[1],
            amount,
            minReturn,
        ...
        require(returnAmount >= minReturn, "OneSplit: actual return amount is less than minReturn");
        tokens[1].universalTransfer(referral, returnAmount.mul(feePercent).div(1e18));
        tokens[1].universalTransfer(msg.sender, returnAmount.sub(returnAmount.mul(feePercent).div(1e18)));
...
struct Balances {
    ...
    function _getFirstAndLastBalances(IERC20[] memory tokens, bool subValue) internal view returns(Balances memory) {
        return Balances({
            ofFromToken: tokens[0].universalBalanceOf(address(this)).sub(subValue ? msg.value : 0),
            ofDestToken: tokens[1].universalBalanceOf(address(this))
        });
    }
    function _swapFloor(
        IERC20 fromToken,

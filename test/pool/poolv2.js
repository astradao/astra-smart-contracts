...
const {
  abi: itokenABI,
} = require("../../artifacts/main/version-5/itoken.sol/itoken.json");
...
describe("Indices", () => {
  ...
  before(async () => {
    ...
    const Oracle = await ethers.getContractFactory("DAAORacle");
  ...
  describe("Configuration Test", function () {
    ...
    describe("Update the Oracle contract address", function () {
      it("Should Revert if non whitelist admin tries to update the  contract address", async function () {
        await expect(
          poolConfiguration.connect(addr1).setOracleaddress(owner.address)
        ).to.revertedWith("Admin only");
      });
      it("Should update the Oracle contract address", async function () {
        await poolConfiguration.setOracleaddress(addr1.address);
        expect(await poolConfiguration.Oraclecontract()).to.be.equal(
          addr1.address
        );
      });
      it("Should not update the Oracle contract address twice", async function () {
        await poolConfiguration.setOracleaddress(addr1.address);
        await expect(
          poolConfiguration.setOracleaddress(addr1.address)
        ).to.revertedWith("setOracleaddress: Already set");
      });
      it("Should update the Oracle contract address", async function () {
        await poolConfiguration.setOracleaddress(addr1.address);
        await poolConfiguration.setOracleaddress(addr2.address);
        expect(await poolConfiguration.Oraclecontract()).to.be.equal(
          addr2.address
        );
      });
  ...
  describe("Add public pool functionality", () => {
    beforeEach(async function(){
      itokenDeployer = await ItokenDeployer.deploy();
      await itokenDeployer.deployed();
    ...
    it("Admin should be able add automatic pool", async () =>{

      await poolConfiguration.setOracleaddress(oracle.address);
      await oracle.whitelistaddress(owner.address)
      await oracle.AddNewPoolTokens([Address.APE_COIN,Address.ASTRA], [2,2],100,100,"First Itoken","ITOKEN1",URL,"Test description")
      await oracle.AddNewPoolTokens([Address.AAVE,Address.ASTRA], [2,2],100,100,"Second Itoken","ITOKEN2",URL,"Test description")
      await pool.addPublicPool([], [],0,0,"","","");

      let itokenAddress = (await pool.poolInfo(1))['itokenaddr'];
      itoken = await erc20iToken(itokenAddress);
      // await itoken.addChefAddress(pool.address);
      expect(await itoken.name()).to.be.equal("Second Itoken");
      expect(await itoken.symbol()).to.be.equal("ITOKEN2");

    })

    it("Non-Admin should not be able add automatic pool", async () =>{
      await poolConfiguration.setOracleaddress(oracle.address);
      await expect(pool.connect(addr2).addPublicPool([], [],0,0,"","","")
      ).to.revertedWith("isAdmin");
  ...
  describe("Deposit in index", () => {
    ...
    it("Should be able to add deposit via ether", async () =>{
      await pool.poolIn([],[],0,{value:ethers.utils.parseUnits('0.1', 18)});
      expect(await daiToken.balanceOf(pool.address)).to.be.equal('117375468076218352082');
    })
  ...
  describe("Withdraw in index", () => {
    ...
    it("Should be able to withdraw the complete amount and exit fees", async () =>{
      ...
      expect(await astraToken.balanceOf(addrs[0].address)).to.be.equal('3092650750294383075634168');
  ...
  describe("Buy token from indices and rebalancing", () => {
    ...
    it("Rebalance", async () =>{
        await pool.updatePool([Address.AAVE,Address.ASTRA],[2,2],0,0, 0);
        expect(await aaveToken.balanceOf(pool.address)).to.equal('10063253688245811103');
        expect(await astraToken.balanceOf(pool.address)).to.be.equal('4070678809317322181862648239');
    ...
    it("Performance fees", async () =>{
      ...
      let totalBalance = await itoken.balanceOf(owner.address);
      await pool.withdraw(0,false,false,totalBalance);
      expect(await astraToken.balanceOf(addrs[0].address)).to.be.equal('105676917614468249569497203');
    })
  })

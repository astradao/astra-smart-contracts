    ...
    describe('TimeLock', function () {
        ...
        describe('executeTransaction (setDelay)', function () {
            beforeEach(async function () {
                ...
                eta = await time.latest(); // Unix timestamp in milliseconds
                eta = parseInt(eta) + parseInt(delay)+parseInt(100);
            ...
            it('requires target.call transaction to succeed', async function () {
                await time.increaseTo(eta);

                await expectRevert(
                    this.timelock.executeTransaction(target, value, signature, revertData, eta, {
                        from: root
                    })
                    , 'Timelock::executeTransaction: Transaction execution reverted.');
            });

            it('requires timestamp to be less than eta plus gracePeriod', async function () {
                await time.increaseTo(parseInt(eta) + parseInt(delay) + parseInt(delay) + parseInt(100));
        ...
        describe('executeTransaction (setPendingAdmin)', function () {
            beforeEach(async function () {
                ...
                await this.timelock.setPendingAdmin(this.timelock.address, { from: root })
                signature = 'acceptAdmin()';
                data = encodeParameters(['address'], [newAdmin]);
                eta = await time.latest(); // Unix timestamp in milliseconds
                eta = parseInt(eta) + parseInt(delay) +parseInt(10);
                // await this.timelock.acceptAdmin(this.timelock.address, { from: root })
                await this.timelock.queueTransaction(target, value, signature, data, eta, {
                    from: root
                });
            });
            ...
            it('sets hash from true to false in queuedTransactions mapping, updates admin, and emits ExecuteTransaction event', async function () {
                const configuredPendingAdminBefore = await this.timelock.pendingAdmin();
                expect(configuredPendingAdminBefore).to.be.equal(this.timelock.address);
                ...
                const configuredAdminAfter = await this.timelock.admin();
                expect(configuredAdminAfter).to.be.equal(this.timelock.address);
            });
        });
    });
});
...
contract GovernanceTestArbitrum is Test {
    ...
    function setUp() public {
        ...
        timelock = new Timelock("Ethereum", address(this), 120, axelarGateway, gasService);

        // Setting Arbitrum Chain
        uint eta = 120 + 100;
        uint value = 0;
        address GovernorBetacontract = address(1);
        string memory chain = "Ethereum";
        address target = address(timelock);
        string memory signature = "setL2GovernanceContract(string,address)";
        bytes memory data = abi.encode("arbitrum", GovernorBetacontract);

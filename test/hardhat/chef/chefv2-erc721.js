...
describe("Chef staking with merged staking", function () {
  ...
  describe("Emergency Withdraw", function () {
    beforeEach(async function () {
      ...
      await chef.connect(addr1).deposit( convertToWei(0), 0, 200, true);

      await nft.connect(owner).safeMint(addr1.address, 300);
      await chef.connect(addr1).deposit( convertToWei(0), 0, 300, true);

      await nft.connect(owner).safeMint(addr1.address, 400);
      await chef.connect(addr1).deposit( convertToWei(0), 0, 400, true);
    });

    it("Should not allow emergency withdraw if not owner", async function () {
      await expect(chef.connect(addr1).emergencyNFTWithdraw([200, 300])).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );
    });

    it ("Should allow emergency withdraw if owner", async function () {
      await chef.connect(owner).emergencyNFTWithdraw([200]);
      expect(await nft.ownerOf(200)).to.be.equal(owner.address);
    });

    it ("Should allow multiple emergency withdraw if owner", async function () {
      await chef.connect(owner).emergencyNFTWithdraw([300, 400]);
      expect(await nft.ownerOf(300)).to.be.equal(owner.address);
      expect(await nft.ownerOf(400)).to.be.equal(owner.address);
    });
  });
});

...
describe("Governance", function () {
  ...
  describe("Emergency Withdraw", function () {
    beforeEach(async function () {
      await chef.connect(addr1).deposit( convertToWei(1000), 0, 0, false);
      await chef.connect(addr2).deposit( convertToWei(2000), 12, 0, false);
    });

    it("Should not allow emergency withdraw if not called by owner", async function () {
      await expect(
        chef.connect(addr1).emergencyWithdraw(convertToWei(3000))
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });

    it("Should allow emergency withdraw if called by owner", async function () {
      let ownerBeforeBalance = await astra.balanceOf(owner.address);
      await chef.connect(owner).emergencyWithdraw(convertToWei(3000));
      let ownerAfterBalance = await astra.balanceOf(owner.address);
      expect(ownerAfterBalance.sub(ownerBeforeBalance)).to.equal(convertToWei(3000));
    });

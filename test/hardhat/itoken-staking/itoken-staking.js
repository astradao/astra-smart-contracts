...
describe("iToken staking", function () {
  ...
  beforeEach(async function () {
    ...
    await itokenDeployer.addChefAddress(itokenstaking.address)
  ...
  describe("Deployment", function () {
    ...
    it("Should whitelist the itokenstaking address in itoken implementation contarct", async function () {

      let itokenDeployed = await itokenDeployer.getcoin(0);
    itoken = await itokenContract.attach(itokenDeployed);

      expect(await itoken.whitelistedaddress(itokenstaking.address)).to.equal(true);
    });
  ...
  describe("Emergency iToken Withdraw", function () {
    beforeEach(async function () {
      await itokenstaking.connect(addr1).deposit( 0, convertToWei(100), 0);
      await itokenstaking.connect(addr2).deposit( 0, convertToWei(100), 0);
    });

    it("Should not allow to withdraw iToken if not owner", async function () {
      await expect(
        itokenstaking.connect(addr1).emergencyWithdraw(0, convertToWei(100))
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });

    it("Should allow to withdraw iToken if owner", async function () {
      const balanceBefore = await itoken.balanceOf(itokenstaking.address);
      await itokenstaking.emergencyWithdraw(0, convertToWei(200));
      const balanceAfter = await itoken.balanceOf(itokenstaking.address);
      expect(differenceOfLargeNumbers(balanceBefore, balanceAfter)).to.equal(convertToWei(200));
    });
    
  });

  describe("Emergency Astra Withdraw", function () {
    beforeEach(async function () {
    });

    it("Should not allow to withdraw Astra if not owner", async function () {
      await expect(
        itokenstaking.connect(addr1).emergencyAstraWithdraw(convertToWei(100))
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });

    it("Should allow to withdraw Astra if owner", async function () {
      const balanceBefore = await astra.balanceOf(itokenstaking.address);
      await itokenstaking.emergencyAstraWithdraw(convertToWei(200));
      const balanceAfter = await astra.balanceOf(itokenstaking.address);
      expect(differenceOfLargeNumbers(balanceBefore, balanceAfter)).to.equal(convertToWei(200));
    });
  });
